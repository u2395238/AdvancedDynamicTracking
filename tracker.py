...
def process_video(source, opt, traffic_light_interval, traffic_light_threshold):
    opt.source = source
    with torch.no_grad():
        detect(opt, traffic_light_interval, traffic_light_threshold)
...
def detect(opt, traffic_light_interval, traffic_light_threshold):
    ...
    last_traffic_light_change = time.time()
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                ...
                try :
                    cv2.imshow('Shalu Traffic Management', im0)
        ...
        if time.time() - last_traffic_light_change > traffic_light_interval:
            if down_count > traffic_light_threshold:
                traffic_light_state = control_traffic_light(traffic_light_state)
                last_traffic_light_change = time.time()
        # Draw traffic light status
        cv2.putText(im0, f'Traffic Light: {traffic_light_state}', (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--source', type=str, default=['input.mp4', 'input1.mp4'], help='source')  # file/folder, 0 for webcam
    ...
    video_sources = ['input.mp4', 'input1.mp4']
    traffic_light_interval = 60  # seconds
    traffic_light_threshold = 10  # adjust as needed
    ...
    #     opt.source = source  # Update the source for each iteration
    #     with torch.no_grad():
    #         detect(opt)

    with multiprocessing.Pool(processes=len(video_sources)) as pool:
        # Use partial to pass the same opt to each process
        process_func = partial(process_video, opt=opt, traffic_light_interval=traffic_light_interval, traffic_light_threshold=traffic_light_threshold)
        pool.map(process_func, video_sources)
